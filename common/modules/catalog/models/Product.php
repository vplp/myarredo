    ...
    public function beforeSave($insert)
    {
        ...
        if (in_array($this->scenario, ['frontend', 'backend']) && !$this->isNewRecord && $this->is_composition == 0 && !in_array(14, $this->category_ids)) {
            if (Yii::$app->request->getBodyParam('SpecificationValue')) {
                ...
                if ($count < 2) {
                    Yii::$app->session->setFlash('error', Yii::t('app', 'Не заполнены размеры и вы не можете сохранить товар'));
                    return false;
                }
            } else {
                Yii::$app->session->setFlash('error', Yii::t('app', 'Не заполнены размеры и вы не можете сохранить товар'));
        ...
        if (isset($this->factory_catalogs_files_ids) && !empty($this->factory_catalogs_files_ids)) {
            $factory_catalogs_files_ids = [];
        ...
        if (isset($this->factory_prices_files_ids) && !empty($this->factory_prices_files_ids)) {
            $factory_prices_files_ids = [];
