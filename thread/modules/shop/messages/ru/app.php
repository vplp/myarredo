<?php

/**
 * @author FilamentV <vortex.filament@gmail.com>
 * @copyright (c) 2015, Thread
 */
return [
    'shop' => 'Магазин',
    'Delivery Methods' => 'Методы доставки',
    'Payment Methods' => 'Методы оплаты',
    'Order' => 'Заказы',
    //Order
    'Full name' => 'Ф.И.О.',
    'Manager' => 'Менеджер',
    'Delivery price' => 'Цена доставки',
    'Order status' => 'Статус заказа',
    'Payd status' => 'Статус оплаты',
    'Count of items' => 'Количество товаров',
    'Summ of items count' => 'Количество позиций',
    'Summ of items without discOrder statusount for item' => 'Сумма без скидки',
    'Total Summ of items with discount for item' => 'Итого без скидки на заказ',
    'Percent discount for order' => 'Процентная скидка на заказ',
    'Discount of money for order' => 'Денежная скидка на заказ',
    'Summ discount for order' => 'Скидка на заказ',
    'Finish Summ' => 'Итого',
    //
    'Работа с заказом' => 'Работа с заказом',
    'Товары заявки' => 'Товары заявки',
    'Цена' => 'Цена',
    'Комментарий' => 'Комментарий',
    'Напоминание' => 'Напоминание',
    'Обработано' => 'Обработано',
    'Дата' => 'Дата',
    'Вернуться к заявкам' => 'Вернуться к заявкам',
    'Все заявки этого клиента' => 'Все заявки этого клиента',
    'Order_status_new' => 'Новая',
    'Order_status_in_work' => 'В работе',
    'Order_status_given_to_salon' => 'Передана в салон',
    'Order_status_contract_signed' => 'Заключен договор',
    'Order_status_failure' => 'Отказ',
    'Order_status_archive' => 'Архивная',
    'Добавить комментарий' => 'Добавить комментарий',
    'Добавить напоминание' => 'Добавить напоминание'
];
